...
const statusMeta = {
  locked: {
    label: "Locked",
    icon: Lock,
    className: "border-white/8 opacity-55 grayscale",
  },
  unlocked: {
    label: "Unlocked",
    icon: UnlockKeyhole,
    className: "border-[#F1C75B]/55 hover:-translate-y-1 hover:shadow-[0_18px_55px_rgba(241,199,91,0.2)]",
  },
  completed: {
    label: "Completed",
    icon: CheckCircle2,
...
export default function DoorCard({ door, onClick }) {
  ...
  return (
    <button
      ...
      disabled={!canOpen}
      className={`group relative min-h-[310px] overflow-hidden rounded-[8px] border bg-[#121922] p-4 text-left transition ${meta.className}`}
    >
      ...
      <div className="relative z-10 flex h-full min-h-[278px] flex-col justify-between">
        <div className="flex items-center justify-between gap-3">
          <span className="rounded-full border border-white/12 bg-black/25 px-3 py-1 text-xs font-black uppercase tracking-[0.16em] text-white/78">
            {door.chapter}
          </span>
